...
- 2026-04-05: Fixed GoalLoop-company fallback routing for assigned non-goal execution issues in `server/src/services/heartbeat.ts`. This closes the live Titan gap where agents like `UXDesigner` were unpaused and had assigned `todo` work, but timer/manual wakes were being skipped as `no_actionable_goal_work` because the company default mode was `goal_loop`. Added focused regression coverage in `server/src/__tests__/heartbeat-routing.test.ts`. Checks: local `pnpm -r typecheck`; VPS `pnpm --filter @paperclipai/server exec vitest run src/__tests__/heartbeat-routing.test.ts`; VPS `pnpm -r typecheck`; VPS `pnpm build`; `systemctl restart paperclip.service`; live validation showed `TIT-314` moved to `in_progress` under UXDesigner heartbeat run `35cfe2bc-325a-4a53-96b1-e7be1d6d99b2`.
